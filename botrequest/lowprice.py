...
def get_result(
    ...
    for i_hotel in hotels_list:
        if i_hotel['ratePlan']['price']['exactCurrent'] <= min_price or len(final_hotels_list) < max_hotel_num:
            if i_hotel['ratePlan']['price']['exactCurrent'] < min_price:
                min_price = i_hotel['ratePlan']['price']['exactCurrent']
            final_hotels_list.append(i_hotel)
            if len(final_hotels_list) > max_hotel_num:
                max_price = -1
                index = None
                for i_elem in final_hotels_list:
                    if i_elem['ratePlan']['price']['exactCurrent'] > max_price:
                        max_price = i_elem['ratePlan']['price']['exactCurrent']
